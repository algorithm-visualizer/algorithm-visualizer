...
const showSuccessToast = createAction(
  `${prefix}/SHOW_SUCCESS_TOAST`,
  (message) => ({ type: "success", message })
);
const showErrorToast = createAction(
  `${prefix}/SHOW_ERROR_TOAST`,
...
export default handleActions(
  {
    [combineActions(showSuccessToast, showErrorToast)]: (
      state,
      { payload }
    ) => {
      const id = uuid.v4();
