...
class Header extends BaseComponent {
  ...
  saveGist() {
    ...
    const { scratchPaper, titles, files, lastFiles, editingFile } =
      this.props.current;
    const gist = {
      ...
      if (scratchPaper && scratchPaper.login) {
        if (scratchPaper.login === user.login) {
          ...
          return GitHubApi.forkGist(scratchPaper.gistId).then((forkedGist) =>
            GitHubApi.editGist(forkedGist.id, gist)
          );
        }
      }
    ...
    save(gist)
      ...
      .then((newScratchPaper) => {
        ...
        this.props.setEditingFile(
          newScratchPaper.files.find((file) => file.name === editingFile.name)
        );
